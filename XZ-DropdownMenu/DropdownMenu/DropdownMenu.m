...
- (void)showMenu:(NSInteger)index {
    ...
    _btnselectedindex = _buttonSelectedIndex;
...
- (void)selectedFirstValue:(NSString *)first SecondValue:(NSString *)second{
    ...
    if (first == nil) {
        index = [NSString stringWithFormat:@"%@-%@", @"x", second];
    }
    //[_buttonIndexArray setObject:index atIndexedSubscript:_buttonSelectedIndex];
    UIButton *btn = (UIButton *)[self.view viewWithTag:_lastIndex];
    [btn setTitle:[NSString stringWithFormat:@"%@%@",first?first:@"",second] forState:UIControlStateNormal];
